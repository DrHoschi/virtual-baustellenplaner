// Baustellenplaner – Demo (v3.9) – ES-Module (importmap: three + addons)
// Profi-Marker: Outline (Edges), tappbar, Stack/Counter pro Bauteil
...
(function(){
  ...
  // Bauteil-Filter (wenn Marker angetippt wird)
  let issuesElementFilterId = null;
  let tasksElementFilterId  = null;

  function openIssuesOverlayForElement(elementId){
    issuesElementFilterId = elementId || null;
    issuesOverlay.classList.remove("hidden");
    renderIssuesOverlay();
  }
  function openTasksOverlayForElement(elementId){
    tasksElementFilterId = elementId || null;
  ...
  function renderIssuesOverlay(){
    ...
    if(issuesElementFilterId){
      const label = `Bauteil: ${issuesElementFilterId} ✕`;
      issuesFilters.appendChild(mkChip(label, true, ()=>{
        issuesElementFilterId = null;
        renderIssuesOverlay();
      }));
    }
    });
  ...
  function renderTasksOverlay(){
    ...
    if(tasksElementFilterId){
      const label = `Bauteil: ${tasksElementFilterId} ✕`;
      tasksFilters.appendChild(mkChip(label, true, ()=>{
        tasksElementFilterId = null;
        renderTasksOverlay();
      }));
    }
    });
  ...
  function clearAllOutlines(){
    // Entfernt alle Outline-Objekte, die wir an Meshes gehängt haben
    hallGroup.traverse((obj)=>{
      if(!obj || !obj.isMesh) return;
      const arr = obj.userData && obj.userData.__outlines;
      if(Array.isArray(arr)){
        arr.forEach(o=>{ try{ obj.remove(o); o.geometry?.dispose?.(); o.material?.dispose?.(); }catch(_){} });
      }
      if(obj.userData) obj.userData.__outlines = null;
    });
  }

  function makeBadgeSprite(text, bgColor){
    // Canvas → Texture → Sprite (immer zur Kamera)
    const w=96, h=48;
    ...
    // Optische Größe (in Meter)
    spr.scale.set(2.2, 1.1, 1.0);
    return spr;
  }

  function addEdgesOutline(mesh, colorHex){
    // Outline über Kanten – wirkt „profi“ ohne Postprocessing
    // (OutlinePass wäre möglich, ist aber auf iOS oft fragiler / schwerer)
    const geo = new THREE.EdgesGeometry(mesh.geometry, 20);
    const mat = new THREE.LineBasicMaterial({ color: colorHex, transparent:true, opacity:0.95, depthTest:true });
    const line = new THREE.LineSegments(geo, mat);
    line.renderOrder = 999; // über Material
    // leicht „aufblasen“ gegen Z-Fighting
    line.scale.multiplyScalar(1.002);

    if(!mesh.userData) mesh.userData = {};
    if(!Array.isArray(mesh.userData.__outlines)) mesh.userData.__outlines = [];
    mesh.userData.__outlines.push(line);
    mesh.add(line);
    return line;
  }
  ...
  function rebuildMarkers(){
    ...
    // Sammle ALLE Issues/Tasks pro Bauteil
    const issuesByEl=new Map();
    for(const it of issuesForActive()){
      if(!it.elementId) continue;
      if(!issuesByEl.has(it.elementId)) issuesByEl.set(it.elementId, []);
      issuesByEl.get(it.elementId).push(it);
    }
    const tasksByEl=new Map();
    for(const tk of tasksForActive()){
      if(!tk.elementId) continue;
      if(!tasksByEl.has(tk.elementId)) tasksByEl.set(tk.elementId, []);
      tasksByEl.get(tk.elementId).push(tk);
    }

    // Prioritäten: Neu/Offen = „schlimmer“
    const prioIssue = (s)=> (s==="Neu"?3 : s==="In Arbeit"?2 : 1);
    const prioTask  = (s)=> (s==="Offen"?3 : s==="In Arbeit"?2 : 1);

    // Hilfsfunktion: Marker Position über Bauteil
    function markerPosForObject(obj, yOffset){
      const box=new THREE.Box3().setFromObject(obj);
      const topY=box.max.y;
      const center=new THREE.Vector3(); box.getCenter(center);
      return new THREE.Vector3(center.x, topY + (yOffset||0.45), center.z);
    }
    ...
    hallGroup.traverse((obj)=>{
      ...
      const its = issuesByEl.get(id) || [];
      const tks = tasksByEl.get(id)  || [];

      if(!its.length && !tks.length) return;

      // Bestimme Outline-Farbe aus dem „schlimmsten“ Status (Issue > Task)
      let outlineColor = 0x2ecc71; // grün default
      if(its.length){
        const top = its.reduce((a,b)=> prioIssue(a.status)>=prioIssue(b.status)?a:b);
        outlineColor = statusColorIssue(top.status);
      }else if(tks.length){
        const top = tks.reduce((a,b)=> prioTask(a.status)>=prioTask(b.status)?a:b);
        outlineColor = statusColorTask(top.status);
      }

      // Outline direkt am Mesh
      addEdgesOutline(obj, outlineColor);

      // --- Marker: Badges (Issues + Tasks) ---
      const basePos = markerPosForObject(obj, 0.55);

      if(its.length){
        const spr = makeBadgeSprite(its.length, "#d9534f"); // rot
        spr.position.copy(basePos);
        spr.userData = { markerOpen:"issues", elementId:id };
        markerGroup.add(spr);
      }
      if(tks.length){
        const spr = makeBadgeSprite(tks.length, "#2b6cb0"); // blau
        spr.position.copy(basePos);
        spr.position.y += 0.95; // stack nach oben
        spr.userData = { markerOpen:"tasks", elementId:id };
        markerGroup.add(spr);
      }
    });
  }
  ...
  function pickAt(clientX, clientY){
    ...
    // 1) Erst Marker treffen (tappbar)
    const mhits = raycaster.intersectObjects(markerGroup.children, true);
    if(mhits.length){
      const o = mhits[0].object;
      const ud = o.userData || {};
      if(ud.markerOpen && ud.elementId) return { kind:"marker", open:ud.markerOpen, elementId:ud.elementId };
    }
    ...
    for(const h of hits){
      const o=h.object;
      if(o && o.userData && o.userData.id) return { kind:"mesh", object:o };
    }
    return { kind:"mesh", object:hits[0].object };
  }
  ...
  renderer.domElement.addEventListener("pointerdown",(e)=>{
    ...
    // Marker antippen => direkt Liste/Modal öffnen
    if(hit.kind==="marker"){
      if(hit.open==="issues") openIssuesOverlayForElement(hit.elementId);
      else if(hit.open==="tasks") openTasksOverlayForElement(hit.elementId);
      return;
    }

    // Bauteil antippen im jeweiligen Modus
